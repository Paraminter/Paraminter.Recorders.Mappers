...
public sealed class ArgumentDataRecorderFactory : IArgumentDataRecorderFactory
{
    ...
    IArgumentDataRecorder<TParameter, TArgumentData> IArgumentDataRecorderFactory.Create<TParameter, TRecord, TArgumentData>(IParameterMapper<TParameter, TRecord, TArgumentData> mapper, TRecord dataRecord)
    {
        if (mapper is null)
        {
    ...
    private sealed class ArgumentDataRecorder<TParameter, TRecord, TArgumentData> : IArgumentDataRecorder<TParameter, TArgumentData>
    {
        private readonly IParameterMapper<TParameter, TRecord, TArgumentData> Mapper;
        private readonly TRecord DataRecord;

        public ArgumentDataRecorder(IParameterMapper<TParameter, TRecord, TArgumentData> mapper, TRecord dataRecord)
        {
            Mapper = mapper;
            DataRecord = dataRecord;
        }

        bool IArgumentDataRecorder<TParameter, TArgumentData>.TryRecordData(TParameter parameter, TArgumentData argumentData)
        {
