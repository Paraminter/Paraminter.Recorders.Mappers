...
internal sealed class RecorderContext<TRecord> where TRecord : class
{
    public static RecorderContext<TRecord> Create()
    {
        ...
        var recorder = ((ISemanticRecorderFactory)factory).Create(mapperMock.Object, dataRecord).Constructor;
    ...
    private RecorderContext(ISemanticConstructorRecorder recorder, Mock<ISemanticMapper<TRecord>> mapperMock, TRecord dataRecord, Mock<ISemanticRecorderLoggerFactory> loggerFactoryMock)
    {
        Recorder = recorder;
