...
public sealed class TryRecordArgument
{
    ...
    public void NullReturningMapper_ReturnsFalseAndLogs()
    {
        ...
        context.MapperMock.Setup(static (mapper) => mapper.Named.TryMapParameter(It.IsAny<string>(), It.IsAny<object>())).Returns((ISyntacticMappedNamedRecorder?)null);
    ...
    private static void ValidRecorder_PropagatesReturnValue(bool recorderReturnValue)
    {
        ...
        context.MapperMock.Setup(static (mapper) => mapper.Named.TryMapParameter(It.IsAny<string>(), It.IsAny<object>())!.TryRecordArgument(It.IsAny<ExpressionSyntax>())).Returns(recorderReturnValue);

        var outcome = Target(context.Recorder, parameterName, syntax);

        Assert.Equal(recorderReturnValue, outcome);

        context.MapperMock.Verify((mapper) => mapper.Named.TryMapParameter(parameterName, context.DataRecordMock.Object)!.TryRecordArgument(syntax), Times.Once);
    }
}
